...
def text_analysis(input_row, model):
    ...
    total_cost = COSTS[model]["input"] * tokens["input_tokens"] + COSTS[model]["output"] * tokens["output_tokens"]
...
def merge_two_summary_datasets(
    ...
    df1_pd = df1_w_stats["train"].to_pandas()[
        ["patient_id", "model_input", "input_flesch_reading_ease", "input_syllables_per_word",
         "input_words_per_sentence", "summary", "summary_flesch_reading_ease", "summary_syllables_per_word",
         "summary_words_per_sentence", "total_cost"]
    ].rename(
        columns={
    ...
    df2_pd = df2_w_stats["train"].to_pandas()[
        ["patient_id", "summary", "summary_flesch_reading_ease", "summary_syllables_per_word",
         "summary_words_per_sentence", "total_cost"]
    ].rename(
        columns={
    ...
    dataset_pd = df1_pd.merge(
        df2_pd, on=["patient_id"]
    )[
        ["patient_id",
         "model_input",
